...
import { getLiveCalls } from "@/lib/data/live-calls"
...
export function LiveCallsPage() {
  ...
  const handleRefreshNow = useCallback(async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        toast({
          title: "Error",
          description: "Failed to refresh live calls.",
          variant: "destructive",
        })
      }
      setIsLoading(false)
  ...
  const handleEndCallConfirm = useCallback(async () => {
    ...
    try {
      ...
      setCalls((prev: LiveCall[]) => prev.filter((c) => c.callId !== callId))
      setDrawerOpen(false)
      setSelectedCall(null)
      toast({
        title: "Call Ended",
  ...
  const handleHandoffConfirm = useCallback(
    async (target: string, createAction: boolean) => {
      ...
      try {
        ...
        // Update call state to handoff_requested
          ...
          prev.map((c) =>
            c.callId === callId
              ? {
                  ...
                  lastEvent: "Handoff requested",
                  riskFlags: [...c.riskFlags.filter((f) => f !== "handoff_requested"), "handoff_requested"],
                  timeline: [
                    ...c.timeline,
                    {
                      ...
                      description: `Handoff requested to ${target}`,
                    },
                  ],
                }
              : c,
        ...
        // If selectedCall is the one being transferred, update it too
          ...
          setSelectedCall((prev: LiveCall | null) =>
            prev
              ? {
                  ...
                  state: "handoff_requested" as const,
                  lastEvent: "Handoff requested",
                  riskFlags: [...prev.riskFlags.filter((f) => f !== "handoff_requested"), "handoff_requested"],
                }
              : null,
          )
        }
    ...
    [handoffCallId, selectedCall, toast],
  )
  ...
  const handleSaveNote = useCallback((callId: string, note: string) => {
    ...
    setCalls((prev: LiveCall[]) =>
      prev.map((c) => (c.callId === callId ? { ...c } : c)),
    )
    toast({
      title: "Note saved",
  ...
  const handleSaveTags = useCallback((callId: string, tags: string[]) => {
    setCalls((prev: LiveCall[]) =>
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (call) =>
          ...
          ((call as any).intent || "").toLowerCase().includes(query),
      )
    }
          ...
          <LiveCallsTable
            ...
            onViewHistory={() => {
              ...
              toast({
                ...
                description: "This would navigate to the Call History page",
              })
            }}
