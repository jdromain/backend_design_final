...
export async function getBillingUsage(period: string) {
  ...
  const raw = await get<ApiBillingUsage>(`/billing/usage?period=${encodeURIComponent(apiPeriod)}`)
...
export async function getBillingBreakdown(period: string) {
  ...
  const raw = await get<ApiBreakdown>(`/billing/breakdown?period=${encodeURIComponent(apiPeriod)}`)
...
export async function getAgentUsage(): Promise<AgentUsageState | null> {
  ...
  const rows = await get<ApiBillingAgentRow[]>("/billing/agents")
...
export async function getToolUsage(): Promise<ToolUsageRow[]> {
  ...
  const rows = await get<ApiToolRow[]>("/billing/tools")
...
export async function getInvoices(): Promise<Invoice[]> {
  ...
  const rows = await get<ApiUsageRollup[]>("/billing/invoices")
  if (!Array.isArray(rows)) return []
