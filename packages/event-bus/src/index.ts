...
import { EventEnvelope, EventPayloadByType, EventType, TypedEventEnvelope } from "@rezovo/core-types";
  ...
  return {
    async publish<E extends EventType>(event: TypedEventEnvelope<E>): Promise<void> {
      ...
      for (const subscriber of subscribers) {
        ...
        // Sequential processing preserves per-tenant ordering expectations.
...
export function createEventEnvelope<E extends EventType>(
  params: {
    eventType: E;
    tenantId: string;
